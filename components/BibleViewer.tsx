...
// Parse Bible reference (Chinese or English)
const parseBibleReference = (text: string): { bookId: string; chapter: number; verses?: number[] } | null => {
  ...
  const englishBookNames = BIBLE_BOOKS.map(b => {
    ...
    return parts.slice(1).join(' '); // Get English name
  }).join('|');
