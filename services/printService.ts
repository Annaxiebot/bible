import { NoteData, JournalEntry } from '../types';
...
/** Generate HTML for printing journal entries */
...
.entry-content h2, .entry-content h3 { color: #1a1a2e; margin: 12px 0 6px; }
.entry-content blockquote { border-left: 3px solid #6366f1; padding-left: 12px; color: #555; margin: 8px 0; }
.entry-content ul, .entry-content ol { padding-left: 24px; margin: 6px 0; }
...
<div class="stats no-print">
  ...
  <span class="stat"><span class="stat-n">${entries.filter(e => e.bibleReference).length}</span> with Bible ref</span>
</div>`;
  ...
  entries.forEach(entry => {
    ...
    if (entry.bibleReference) {
      html += `<span class="entry-ref">${entry.bibleReference}</span>`;
    }
    if (entry.tags && entry.tags.length > 0) {
      html += entry.tags.map(t => `<span class="entry-tag">#${t}</span>`).join(' ');
    }
    html += `</div>`;

    // Content — already HTML from contentEditable
  ...
  html += `<div class="info" style="margin-top:40px;border-top:1px solid #ddd;padding-top:15px;">Personal Journal - The Bible App</div>
</body></html>`;

  return html;
}

/** Open print dialog for journal entries */
